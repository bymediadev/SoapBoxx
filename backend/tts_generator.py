...
class TTSGenerator:
    ...
    def _get_supported_voices(self) -> Dict[str, List[str]]:
        ...
        voices = {
            ...
            "azure": ["en-US-AriaNeural", "en-US-GuyNeural", "en-US-JennyNeural", "en-US-DavisNeural"],
            "local": ["default"],
    ...
    def generate_speech(self, text: str, voice: Optional[str] = None, 
        ...
        try:
            if not text.strip():
                track_api_error("Empty text provided for TTS generation", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.MEDIUM)
                return None
            ...
            if self.service == "openai":
                ...
                track_api_error(f"Unsupported TTS service: {self.service}", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
                return None
                
        except Exception as e:
            track_api_error(f"TTS generation failed: {str(e)}", 
                          category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
            return None
    
    def _generate_openai_speech(self, text: str, voice: str, output_path: str, speed: float) -> Optional[str]:
        """Generate speech using OpenAI TTS"""
        try:
            if not OPENAI_AVAILABLE:
                track_api_error("OpenAI TTS not available", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
                return None
            ...
            track_api_error(f"OpenAI TTS generation failed: {str(e)}", 
                          category=ErrorCategory.AI_API, severity=ErrorSeverity.HIGH)
            return None
    
    def _generate_azure_speech(self, text: str, voice: str, output_path: str, speed: float) -> Optional[str]:
        """Generate speech using Azure Speech TTS"""
        try:
            if not AZURE_AVAILABLE:
                track_api_error("Azure Speech TTS not available", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
                return None
            ...
            if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                ...
                track_api_error(f"Azure TTS failed: {result.reason}", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
                return None
                
        except Exception as e:
            track_api_error(f"Azure TTS generation failed: {str(e)}", 
                          category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
            return None
    
    def _generate_local_speech(self, text: str, voice: str, output_path: str, speed: float) -> Optional[str]:
        """Generate speech using local TTS (pyttsx3)"""
        try:
            if not PYTTSX3_AVAILABLE:
                track_api_error("Local TTS not available", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
                return None
            
            # Configure local engine
            self.local_engine.setProperty('rate', int(200 * speed))  # Default rate is 200
            ...
            track_api_error(f"Local TTS generation failed: {str(e)}", 
                          category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
            return None
    
    def _generate_google_speech(self, text: str, voice: str, output_path: str, speed: float) -> Optional[str]:
        """Generate speech using Google TTS"""
        try:
            if not GTTS_AVAILABLE:
                track_api_error("Google TTS not available", 
                              category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
                return None
            ...
            track_api_error(f"Google TTS generation failed: {str(e)}", 
                          category=ErrorCategory.TTS, severity=ErrorSeverity.HIGH)
            return None
    ...
    def test_service(self) -> bool:
        ...
        try:
            ...
            track_api_error(f"TTS service test failed: {str(e)}", 
                          category=ErrorCategory.TTS, severity=ErrorSeverity.MEDIUM)
            return False


# Convenience functions
def create_tts_generator(service: str = "openai", api_key: Optional[str] = None) -> TTSGenerator:
    """Create a TTS generator instance"""
    return TTSGenerator(service=service, api_key=api_key)


def generate_speech_from_text(text: str, service: str = "openai", 
