...
from PyQt6.QtWidgets import (QButtonGroup, QGridLayout, QGroupBox, QHBoxLayout,
    ...
    def run(self):
        ...
        try:
            ...
            results = {
                ...
                "duration_estimate": len(transcript.split()) / 150  # Rough estimate: 150 words per minute
            }
...
class ReverbTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Description
        description = QLabel("AI-powered feedback and coaching tools to help you create better podcasts")
        ...
        self.file_path_label.setStyleSheet("color: #666; padding: 5px; border: 1px solid #ccc; border-radius: 3px;")
        ...
        content_btn.setEnabled(api_keys.get("OpenAI API Key") and api_keys.get("OpenAI API Key") != "Not set")
        ...
        video_analysis_btn.setEnabled(api_keys.get("YouTube API Key") and api_keys.get("YouTube API Key") != "Not set")
        ...
        video_research_btn.setEnabled(api_keys.get("YouTube API Key") and api_keys.get("YouTube API Key") != "Not set")
        ...
        coaching_btn.setEnabled(api_keys.get("OpenAI API Key") and api_keys.get("OpenAI API Key") != "Not set")
        ...
        engagement_btn.setEnabled(api_keys.get("OpenAI API Key") and api_keys.get("OpenAI API Key") != "Not set")
        ...
        storytelling_btn.setEnabled(api_keys.get("OpenAI API Key") and api_keys.get("OpenAI API Key") != "Not set")
        ...
        self.results_text.setPlaceholderText("Episode analysis results will appear here...")
    ...
    def select_episode_file(self):
        ...
        if file_path:
            ...
            self.file_path_label.setStyleSheet("color: #333; padding: 5px; border: 1px solid #4CAF50; border-radius: 3px; background-color: #E8F5E8;")
    ...
    def analyze_episode(self):
        ...
        self.analysis_thread = EpisodeAnalysisThread(self.selected_file_path, analysis_type)
    ...
    def on_analysis_complete(self, results):
        ...
        episode_item = QListWidgetItem(f"📁 {results['file_name']} - {results['analysis_type']}")
    ...
    def on_analysis_error(self, error):
        ...
        self.analyze_btn.setText("🔍 Analyze Episode")
        self.analysis_progress.setVisible(False)
        QMessageBox.critical(self, "Analysis Error", f"Episode analysis failed: {error}")
    ...
    def display_analysis_results(self, results):
        ...
        try:
            ...
            output += f"⏱️ Estimated Duration: {results['duration_estimate']:.1f} minutes\n"
    ...
    def video_content_analysis(self):
        ...
        try:
            ...
            if not google_apis.is_available():
                self.results_text.setText("❌ YouTube API not configured. Please set YOUTUBE_API_KEY in your .env file")
                return
            
            # Search for video podcast content for analysis
            self.results_text.setText("🎥 Video Content Analysis\n\nSearching for video podcast content to analyze...")
            
            # Get trending video podcasts
            trending_results = google_apis.get_youtube_trends("US", max_results=5)
            
            if "error" in trending_results:
                self.results_text.setText(f"❌ Error fetching video content: {trending_results['error']}")
                return
            ...
            for i, video in enumerate(trending_results.get("videos", [])[:3], 1):
                ...
                description = video.get("description", "")[:100] + "..." if len(video.get("description", "")) > 100 else video.get("description", "")
            ...
            analysis_results.append(f"\n🎯 Key Insights:")
            analysis_results.append(f"• Trending content shows current audience interests")
            ...
            analysis_results.append(f"• High view counts indicate successful content strategies")
    ...
    def video_podcast_research(self):
        ...
        try:
            ...
            self.results_text.setText("🔍 Video Podcast Research\n\nResearching video podcast trends and content...")
            
            # Search for video podcast content
            search_results = google_apis.search_youtube_simple("video podcast", max_results=5)
            
            if "error" in search_results:
                self.results_text.setText(f"❌ Error searching video podcasts: {search_results['error']}")
                return
            ...
            research_results.append(f"• Multiple formats available (interviews, discussions, etc.)")
            research_results.append(f"• High-quality production increases viewer retention")
    ...
    def podchaser_analytics(self):
        ...
        try:
            ...
            if not podcast_apis.podchaser_key:
                self.results_text.setText("❌ Podchaser API key not configured. Please add PODCHASER_API_KEY to your .env file")
                return
            
            self.results_text.setText("🎯 Podchaser Analytics\n\nFetching podcast analytics from Podchaser...")
            
            # Search for popular podcasts to analyze
            search_results = podcast_apis.search_podcasts("podcast", service="podchaser")
            
            if "error" in search_results:
                self.results_text.setText(f"❌ Error fetching Podchaser data: {search_results['error']}")
                return
            ...
            for i, edge in enumerate(search_results.get("results", [])[:3], 1):
                ...
                analytics_results.append(f"   Rating: {rating}/5 ({review_count} reviews)")
                analytics_results.append(f"   Categories: {', '.join(categories) if categories else 'N/A'}")
            ...
            analytics_results.append(f"• High-rated podcasts show strong audience engagement")
    ...
    def podchaser_trending(self):
        ...
        try:
            ...
            self.results_text.setText("📈 Podchaser Trending\n\nFetching trending podcasts from Podchaser...")
            
            # Get trending podcasts
            trending_results = podcast_apis.get_trending_podcasts(service="podchaser")
            
            if "error" in trending_results:
                self.results_text.setText(f"❌ Error fetching trending podcasts: {trending_results['error']}")
                return
