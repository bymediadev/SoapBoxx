...
def get_runtime_bucket() -> str:
    """
    Return active runtime bucket.

    Supported aliases:
    - demo: demo, dev, development, sandbox
    - production: prod, production, live
    """
    raw = (os.getenv("SOAPBOXX_BUCKET") or "production").strip().lower()
    if raw in {"demo", "dev", "development", "sandbox"}:
        return "demo"
    return "production"


def default_config_path_for_bucket() -> Path:
    """
    Resolve the config path for the current runtime bucket.

    Priority:
    1) SOAPBOXX_CONFIG_FILE
    2) SOAPBOXX_BUCKET=demo -> soapboxx_config.demo.json
    3) fallback -> soapboxx_config.json
    """
    explicit = (os.getenv("SOAPBOXX_CONFIG_FILE") or "").strip()
    if explicit:
        return Path(explicit)
    if get_runtime_bucket() == "demo":
        return Path("soapboxx_config.demo.json")
    return Path("soapboxx_config.json")


def default_runs_root_for_bucket() -> Path:
    """
    Resolve runs root for the current runtime bucket.

    Priority:
    1) SOAPBOXX_RUNS_DIR
    2) SOAPBOXX_BUCKET=demo -> runs/demo
    3) fallback -> runs
    """
    explicit = (os.getenv("SOAPBOXX_RUNS_DIR") or "").strip()
    if explicit:
        return Path(explicit)
    if get_runtime_bucket() == "demo":
        return Path("runs") / "demo"
    return Path("runs")
...
class Config:
    ...
    def __init__(self, config_file: Optional[str] = None):
        self.config_file = (
            Path(config_file) if config_file else default_config_path_for_bucket()
        )
